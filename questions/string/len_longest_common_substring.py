"""
    LEN LONGEST COMMON SUBSTRING (50CIQ 47: LONGEST COMMON SUBSTRING)

    Write a function, which when given two strings, returns the length of the  longest common substring of the two
    arguments.
...
def len_longest_common_substring_naive(a, b):
...
def len_longest_common_substring_memo(a, b):
    if isinstance(a, str) and isinstance(b, str):
        result = 0
            ...
            for r in range(len(b)):
                for c in range(len(a)):
                    if a[c] == b[r]:                                    # If there is a match:
                        ...
                        if memo[r][c] > result:                             # If new matched len is longer than result:
                            result = memo[r][c]                                 # Update result.
        return result
...
def len_longest_common_substring(a, b):
    if isinstance(a, str) and isinstance(b, str):
        result = 0
        if len(a) > 0 and len(b) > 0:
            ...
            for r in range(len(b)):                     # For each char in 'b' (the longer string).
                ...
                for c in range(len(a)):                     # for each char in 'a' (the shorter string).
                    if a[c] == b[r]:                            # If current chars match:
                        ...
                        if curr[c] > result:                        # Update result if needed.
                            result = curr[c]
                prev = curr
        return result
...
args = [('ABAB', 'BABA'),
        ...
        (None, 'abc'),
        (None, None)]
fns = [len_longest_common_substring_naive,
       len_longest_common_substring_memo,
       len_longest_common_substring]
