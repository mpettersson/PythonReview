"""
    ...
    Example:
        ...
        Output = 1287    # Max value set: [(Value: 99, Weight: 10), (Value: 99, Weight: 10), (Value: 99, Weight: 10),
                         #                 (Value: 99, Weight: 10), (Value: 99, Weight: 10), (Value: 99, Weight: 10),
                         #                 (Value: 99, Weight: 10), (Value: 99, Weight: 10), (Value: 99, Weight: 10),
                         #                 (Value: 99, Weight: 10), (Value: 99, Weight: 10), (Value: 99, Weight: 10),
                         #                 (Value: 99, Weight: 10)]
"""
...
# index) until either the end of the list, or there is no more capacity.
#
...
def knapsack_unbounded_bf_via_rec(items, capacity):
    ...
    def _rec(items, i, c):
        ...
        if c >= items[i].weight:
            included = _rec(items, i, c -items[i].weight)
...
def knapsack_unbounded_dp_via_top_down(items, capacity):
    ...
    def _rec(items, i, c, dp, b):
        ...
        if dp[i][c] is None:
            ...
            included = 0 if c < items[i-1].weight else items[i-1].value + _rec(items, i, c-items[i-1].weight, dp, b)
            # NOTE: 01 KNAPSACK recurses on i-1, not i (ie, _rec(items, i, c-items[i-1].weight, dp, b)).
    ...
    if items is not None and capacity is not None and capacity >= 0:
        ...
        i = n                                               # boolean matrix, see the example above for more info.
        c = capacity                                        # NOTE: The reverse order (to start at b[-1][-1])!!
        while i > 0 and c > 0:                              # NOTE: This loop is also different for the 0/1 KNAPSACK...
            if b[i][c]:                                     # If included:
                result_set.append(items[i-1])                   # Add the corresponding item.
                c -= items[i-1].weight                          # Update column/capacity.
            else:                                           # Else:
                i -= 1                                          # Try the next row/item.
        return result_tot, result_set                       # Return the value and set.
...
# Time Complexity: O(n * capacity), where n is the number of the items list.
#
# NOTE: This may appear to be a POLYNOMIAL time for a np-complete (01 knapsack) problem.  HOWEVER, the metrics are
...
def knapsack_unbounded_dp_via_bottom_up(items, capacity):
    if items is not None and capacity is not None and capacity >= 0:
        ...
        for i in range(1, n+1):                             # i=0 is a padding row, i=1 to n are the list items.
            for c in range(capacity+1):                     # c=discrete capacity from 0 to total capacity (inclusive).
                excluded = dp[i-1][c]
                included = 0 if c < items[i-1].weight else items[i-1].value + dp[i][c-items[i-1].weight]
                # NOTE: For 01 KNAPSACK, dp would reference dp[i-1], not dp[i] (ie, dp[i][c-items[i-1].weight]).
                if excluded < included:
                    dp[i][c] = included
                    b[i][c] = True                          # INCLUDE the corresponding item (row - 1).
                else:
                    dp[i][c] = excluded
        result_set = []                                     # The next few lines builds the result set from the
        i = n                                               # boolean matrix, see the example above for more info.
