"""
    ...
    subsequence in the list.  Remember, elements of a increasing subsequence are NOT required to immediately follow each
    other in the given list.
...
# subsequence ending at the respective index.  Previous pointers (prev), which point to the previous element in the
# longest increasing subsequence (that the element is a part of), are also maintained so that the subsequence can be
# built and returned.
#
...
# pointer is also updated.  Once all of the values have been compared, the longest increasing subsequence
...
# element in the longest increasing subsequence, for the example above:
#
...
def find_longest_increasing_subsequence_memo(l):
    if isinstance(l, list):
        ...
        if len(l) > 0:
            ...
            while last is not None:                     # Build the longest increasing subsequence.
...
# values) is used to map a 'card' with the previous 'card' in an increasing subsequence.  If a new pile is needed, the
...
# the immediately preceding 'pile'. Once complete, the longest increasing subsequence is then built, and returned, using
# the previous pointers list.
#
...
# Space Complexity: O(n), where n is the number of elements in the list.
def find_longest_increasing_subsequence(l):
    if isinstance(l, list):
        result = []
        if len(l) > 0:
            top_val = []            # NOTE: Using two lists (as opposed to a (val, idx) tuple) so bisect can be used w/o
            top_idx = []            # being modified (with (val, idx) it'd compare idx too, which isn't what we want).
            prev = [None] * len(l)
            for i, x in enumerate(l):
                pile = bisect_left(top_val, x)      # Binary search to find the 'pile' that the 'card' goes on.
                if pile == len(top_val):
                    top_val.append(x)
                    top_idx.append(i)
                else:
...
lists = [[0, 8, 4, 12, 2, 10, 6, 14, 1, 9],
         ...
         []]
fns = [find_longest_increasing_subsequence_memo,
