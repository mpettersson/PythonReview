r"""
    ...
    Given the root of a tree and an integer k, write a function which returns the kth node in an preorder traversal
...
#   (1) Return root if k == 1,
#   (2) Recurse left if k - 1 <= left size and,
#   (3) Recurse right if k > left.size + 1.
# Time Complexity: O(h), where h is the height of the tree.
# Space Complexity: O(h), where h is the height of the tree.
def get_kth_preorder_node(root, k):

    def _get_kth_preorder_node(root, k):
        if k is 1:                                                      # (1)  k == 1
            return root
        left_size = root.left.size if root.left else 0
        if k - 1 <= left_size:                                          # (2)  k - 1 <= left_size
            return _get_kth_preorder_node(root.left, k - 1)
        return _get_kth_preorder_node(root.right, k - left_size - 1)   # (3)  k > left_size + 1

    if root and k is not None and 0 < k <= root.size:
        return _get_kth_preorder_node(root, k)
