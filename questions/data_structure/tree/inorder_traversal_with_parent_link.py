r"""
    INORDER WITH PARENT LINK   (EPI 10.11: IMPLEMENT AN INORDER TRAVERSAL WITH O(1) SPACE)
...
def inorder_w_parent_link_epi_sol_iter(root):
    if root is not None:
        prev = None
        curr = root
        while curr:
...
# Time Complexity: O(n), where n is the number of nodes in the tree.
# Space Complexity: o(1).
def inorder_with_parent_link_iterative(root):
    if root is not None:
        prev = None
        curr = root
        while curr:
...
fns = [inorder_w_parent_link_epi_sol_iter,
       inorder_with_parent_link_iterative]
