"""
    ...
    Design an algorithm for computing the k largest elements in a list.
...
# Brute Force/Sorting Approach:  Time complexity is O(n log(n)), where n is the length of the list.  Space complexity is
...
def find_k_largest_elements_via_sorted(l, k):
    if l is not None and k is not None and 0 < k <= len(l):
        return sorted(l)[-k:]


# Min Heap Approach:  Time complexity is O(n log(k)), where n is the length of the list.  Space complexity is O(k).
def find_k_largest_elements_via_min_heap(l, k):
    if l is not None and k is not None and 0 < k <= len(l):
        heap = l[:k]
        heapq.heapify(heap)
        for i in l[k:]:
...
# NOTE: Only THREE lines differ with quick select!
...
vals = [None, -10, 0, 1, 2, 3, 98, 99, 100]
fns = [find_k_largest_elements_via_sorted, find_k_largest_elements_via_min_heap,
